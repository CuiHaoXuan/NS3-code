...
{  
  ...
  cmd.Parse (argc, argv);
  RngSeedManager::SetSeed (10); // Changes seed from default of 1 to 3
  RngSeedManager::SetRun (50);
  DoubleValue doubleValue;
  EnumValue enumValue;
  BooleanValue booleanValue;
  StringValue stringValue;
  //uint32_t maxBytes = 1400;
  ...
  Config::SetDefault ("ns3::LbtAccessManager::MinCw", UintegerValue (8));
  Config::SetDefault ("ns3::LbtAccessManager::MaxCw", UintegerValue (64));
  ...
  double rxPowerDbmD3 = plm->CalcRxPower (txPowerFactors,
                                            bsNodesB.Get (0)->GetObject<MobilityModel> (),
                                            ueNodesB.Get (0)->GetObject<MobilityModel> ());
  double rxPowerDbmD4 = plm->CalcRxPower (txPowerFactors,
                                            bsNodesB.Get (0)->GetObject<MobilityModel> (),
                                            ueNodesA.Get (0)->GetObject<MobilityModel> ());
  std::ostringstream simulationParams;
  simulationParams << d1 << " " << d1 << " "
                   << rxPowerDbmD1 << " "
                   << rxPowerDbmD2 << " "
                   << rxPowerDbmD3 << " "
                   << rxPowerDbmD4 << " "
